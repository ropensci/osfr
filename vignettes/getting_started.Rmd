...
In general, files can exist in projects, components, and/or directories on the OSF. Files can be stored on *OSF's Storage* or in another service that is connected to an OSF project (e.g. GitHub, Dropbox, or Google Drive). However, `osfr` currently only supports interacting with files on OSF Storage.
...
osfr is compatible with the dplyr package, so we can use `filter()` to select the component we 
...
The `mtcars_file` object now contains an `osf_tbl` with 1 row corresponding to the version of that file stored on OSF. Again, we could use `osf_open()` to view it directly on OSF.

OSF also version controls files, which means that if we were to make a change to the `mtcars.csv` file (perhaps we collected more data) we could automatically create a 2nd version of the dataset by using the `osf_upload()` function and setting `overwrite = TRUE`. As long as the file we're uploading has the same file name as a file in that particular section of the project, a new version will be created.
...
osf_rm(car_project, recursive = TRUE)
unlink("mtcars.csv")
```
