...
1. Call the `osf_auth()` function in the console at the start of each new session and paste in your PAT. Your PAT functions like a password, so it should **not** be hard coded into any scripts.

2. To automatically authenticate, store you the PAT as an environment variable named `OSF_PAT`. One way to do this is to create a `.Renviron` file in your home or project working directory, and include a definition of the `OSF_PAT` variable.
