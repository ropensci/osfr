...
#' @param \ldots Additional parameters passed to \code{\link{construct_link_files}} and \code{\link{construct_link}}
#'
...
delete_empty <- function(id, ...) {
  ...
  call <- httr::DELETE(url_osf, httr::add_headers(Authorization = sprintf(
    "Bearer %s",
    login())))
